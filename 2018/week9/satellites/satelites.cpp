...
//looking for minimum vertex cover
//bipartite graph
//all unvisited in L and all visited in R
void do_testcases(){
	int g, s, l;
	cin >> g >> s >> l;
	...
	//satelite to target
	REP(i,s){
		eaG.addEdge(g+i, trg, 1);
	}

	//add edges from ground to satelite
	REP(i,l){
		int u, v; cin >> u >> v;
		eaG.addEdge(u, g+v, 1);
	}
	...
	//all unvisited in g and all visited in s
	vector<int> ground;
	vector<int> satelite;
	...
	REP(i,s){
		if(vis[g+i])
		satelite.push_back(i);
	}
	
	cout << ground.size() << " " << satelite.size() << endl;
	
	for(auto grd : ground){
		cout << grd << " ";
	}
	
	for(auto sat : satelite){
		cout << sat << " ";
	}
	cout << endl;
}
