...
void preprocess(int from, Graph& G, int n, vector<vector<int>>& weights, vector<vector<int> > & preprocessed){
    //all vertices not visited
    ...
    while(!dfs.empty()){
        ...
        //for all edges going out of vertex
        for (boost::tie(oebeg, oeend) = boost::out_edges(curr, G); oebeg != oeend; ++oebeg) {
            ...
            if(visited[v])
                continue;
            int w = weights[u][v];
            //most expensive edge on cycle from v
...
void testcase() {
    ...
    vector<vector<int> > preprocessed(n, vector<int>(n, -1));
	// find replacemenet of min cost
    int min = INT32_MAX;
    //for all edges in nonMST (pair<int,int>
    for (auto it = nonMST.begin(); it != nonMST.end(); ++it){
        int w = weights[it->first][it->second]; //u,v edge weight
        //if we did not start to  check cycle from vertex u we do now
        if(preprocessed[it->first][0] == -1){
            preprocess(it->first, G2, n, weights, preprocessed);
        }
        //edge weight minus cancelled edge weight
